...
public class EvaluateChoices {
    ...
    public void add(int[] p) {
        ...
        if (!indices.contains(ix)) {
            ...
            double combinedValue = exploit + kExplore * explore + random.nextDouble() * epsilon;
            // System.out.format("\t %d\t %d\t %.2f\t %.2f\t %.2f\n", i, j, exploit, explore, combinedValue);
            picker.add(combinedValue, p);
        }
        else {
            // System.out.println("Collision: " + Arrays.toString(p));
        }
    }
