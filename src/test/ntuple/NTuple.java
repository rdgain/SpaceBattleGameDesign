...
public class NTuple {
    ...
    public static void main(String[] args) {
      //  spaceBattleGameSearchSpace = SpaceBattleGameSmearchSpace.getSearchSpace();
   //     System.out.println("Battle Game Size: " + SearchSpaceUtil.size(spaceBattleGameSearchSpace));
        // SearchSpaceUtil.

        // TenSpace is a nice easy one for testing
        //spaceBattleGameSearchSpace = new TenSpace(6);

        int[] tuple = new int[]{0, 1, 2};
   //     NTuple nt = new NTuple(spaceBattleGameSearchSpace, tuple);
...
//        nt.printNonEmpty();
    ...
    public NTuple(SearchSpace searchSpace2, int[] tuple) {
        this.searchSpace = searchSpace2;
     //   System.out.println("from NTuple "+searchSpace);
    ...
    public int address(int[] x) {
        ...
        for (int i : tuple) { //  i<tuple.length; i++) {
            if (searchSpace.indexOf(i, x[i]) >= searchSpace.nValues(i)) {
                throw new RuntimeException("Dimension exceeded: " + searchSpace.indexOf(i, x[i]) + " : " + x[i] + " : " + searchSpace.nValues(i));
            }
            addr += prod * searchSpace.indexOf(i, x[i]);
