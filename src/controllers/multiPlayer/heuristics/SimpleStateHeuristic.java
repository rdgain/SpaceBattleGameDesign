...
public class SimpleStateHeuristic extends StateHeuristicMulti {
  ...
  public double evaluateState(StateObservationMulti stateObs, int playerID) {
    ...
    ArrayList<StateObservationMulti>[] npcPositions = stateObs.getNPCPositions(avatarPosition);
    ArrayList<StateObservationMulti>[] portalPositions = stateObs.getPortalsPositions(avatarPosition);
    HashMap<Integer, Integer> resources = stateObs.getAvatarResources(playerID);

    ArrayList<StateObservationMulti>[] npcPositionsNotSorted = stateObs.getNPCPositions();
...
//        }
    ...
    if (npcPositions != null) {
      for (ArrayList<StateObservationMulti> npcs : npcPositions) {
        if(npcs.size() > 0)
        {
...
//          npcCounter += npcs.size();
    ...
    for (ArrayList<StateObservationMulti> portals : portalPositions) {
      if(portals.size() > 0)
      {
